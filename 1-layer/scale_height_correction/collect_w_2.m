...
sigma   = 1;            %   Scaling of horizontal variation

figure ( 1 )
% lid height 10 x heating height
h(1)  =  subplot(3,1,1);
HL_bar   = 10.0;          %   relative lid height
HV_bar   = 10.0;          %   relative visualisation height
...
grid on
axis equal
x = sprintf('b structure at t = %6.1fs, H_L=%6.1f*H_t',t,HL_bar);
title(x);
xlabel('x / FWHM');
ylabel('z / H_t');
...
grid on
axis equal
x = sprintf('b structure t = %6.1fs, H_L=%6.1f*H_t',t,HL_bar);
title(x);
xlabel('x / FWHM');
ylabel('z / H_t');
